...
class Order extends Adminbase
{
    ...
    {
        Adminbase::checkActionAuth("order/index", "view");
        $Order            = model("Order");
        $where            = [];
        ...
        $lists = $Order->where($where)->order('order_addtime', 'desc')->paginate($pagesize);
        if ($lists) {
            $Pay           = model("pay");
        ...
        //统计
        //交易总金额
        $lists["tj_total"] = $Order->where($where)->sum("order_pay_realprice");
        if (!$lists["tj_total"]) {
            $lists["tj_total"] = 0;
        }
        ...
        $lists["tj_alipay_total"]                   = $Order->where($tj_alipay_total_where)->sum("order_pay_realprice");
        ...
        $lists["tj_wxpay_total"]                   = $Order->where($tj_wxpay_total_where)->sum("order_pay_realprice");
        ...
        $lists["total_amount"] = $Order->where($where)->sum("order_total_amount");
    ...
    public function gettjdata()
    {
        ...
        //今日订单数
        $data["tj_count"] = $Order->where($where)->count();
        //今日交易额
        $data["tj_total"] = $Order->where($where)->sum("order_total_amount")?:0;
        //总商户数
        $data["tj_shop_count"] = model("shop")->count();
        //总门店数
        $data["tj_store_count"] = model("store")->count();
        //活跃门店(最近10天)
        $huoyuemendianlist_sql = $Order->where([
        ...
        $huoyuemendianlist         = model("shop")->join("__STORE__", " shop_id = store_shop_id ", "left")->join($huoyuemendianlist_sql . " as qs_order", "order_store_id=store_id", "right")->field("store_name,shop_name,order_total_amount")->select();
        $data["huoyuemendianlist"] = $huoyuemendianlist;


        //登入首页，交易流水走势(15天)，功能
        $yiliushuizoushilist = $Order->where([
    ...
    public function add()
    {
        Adminbase::checkActionAuth("order/index", "add");
        $Order = model("Order");
        $one   = [];
        $this->assign("one", $one);
        return view();
    ...
    {
        Adminbase::checkActionAuth("order/index", "delete");
        $order_id = input("order_id/a");
    }
    public function save()
    {
        ...
        $action   = input("action");
        return ['code' => 1, 'message' => '修改成功!', 'wait' => 1, 'url' => url("add", ["order_id" => $order_id])];
    }
    public function parking_baoyue()
    {
        Adminbase::checkActionAuth("order/index", "add");
    ...
    *By
    */
    public function carmonthlist()
    {
        Adminbase::checkActionAuth("order/index", "add");
        $order_addtime_s  = input("get.order_addtime_s", "");/*包月时间*/
        $keyword          = input("get.keyword", "");/*车牌号*/
        $order_shop_id    = input("get.order_shop_id/d", "");/*商户ID*/
        $order_store_id   = input("get.order_store_id/d", "");/*停车场ID*/
        $pagesize    = input("get.pagesize/d", 10);/*每页数量,默认十个每页*/
        $rs=model('MemberCarRecord')->get_list(['car_month'=>$order_addtime_s,'keyword'=>$keyword,'order_shop_id'=>$order_shop_id,'order_store_id'=>$order_store_id,'pagesize'=>$pagesize]);
        return $rs;    
    }
}