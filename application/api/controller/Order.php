...
class Order extends Apibase
{
    ...
    function list() {
        ...
        */
        ...
        选择除了日期的其他搜索，如果限制了三天就查不到信息，因此先把这个默认限制3天内的订单，这个记录去掉
        */
        // if (!$order_addtime_from) {
        //     $order_addtime_from = date("Y-m-d", strtotime("- 2 days"));
        // }
        ...
        */
        ...
        //订单号
        if ($order_num) {
            $whereOrderSearch["order_num|order_guest_brief"] = ["like", "%" . $order_num . "%"];
            /*     
            *by
            *create date:2018-2-5 15:53
            *搜索关键词，不限制日期，如果传递有日期就放到里面
        ...
        $field = ["order_id", "order_num", "order_addtime","order_guest_brief","order_subject", "order_status", "order_channel_id", "order_total_amount", "order_pay_realprice"];

        $lists = $order->where($whereOrderSearch)->field($field)->order("order_id desc")->paginate($per_page);

        if ($lists) {
            $Pay           = model("pay");
            $order_pay_log = model("order_pay_log");
            foreach ($lists as $listsOne) {
                $paychannel                   = $Pay->payWayTranslate($listsOne->order_channel_id, true);
                $listsOne->order_channel_info = $paychannel["cn"];
                $listsOne->order_channel      = $paychannel["name"];
