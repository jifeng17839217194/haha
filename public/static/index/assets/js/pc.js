...
var thisFrontApp = {
    ...
    sound: {
        ...
        getsoundfile: function(cash_num) {
            ...
            $.ajax({
                ...
                "dataType": "json",
                "success": function(rs) {
                    //soundPlayList.push("http://www.w3school.com.cn/i/horse.ogg");
    ...
    orderchange: function(rs) {
        ...
        var thismarkerlablel = rs.data.order_num + "" + rs.data.trade_status;
        if (order_no_list.indexOf(thismarkerlablel) < 0) {
            window.ordernolist += ',' + thismarkerlablel;
        } else {
            return;
        }
        ...
        switch (rs.data.trade_status) {
            case 100: //成功
                layer.msg(rs.data.pay_realprice + '支付成功', { time: 1500 });
                $("#pay_code").val("");
                $("#pay_price").focus();
                $('.payment-primary').html('应收金额：<small>待输入</small>');


                var html = thisFrontApp.getordertemplate(rs.data);
                console.log('html===' + html);
                if ($('.order-nearly').children('div').length >= 8) {
                ...
                try {
                    if (rs.data.create_where == "pc") {
                        windowForm.jsprint(rs.data.printhtml, rs.data.order_num);
                    }
                } catch (e) {
            ...
            case 500: //未知异常，调用查询接口确认支付结果
                layer.alert('等待输入支付密码<br />稍后在“订单”列表查看结果', {
                    title: "稍等",
                    btn: false,
                    success: function() {
            ...
            case 400: //未知异常,调用查询接口确认支付结果
                ...
                layer.alert(rs.message || "¥" + rs.data.total_amount + "支付已关闭", {
                    success: function() {
                        $(document).on('keypress', function(e) {
                            if (e.keyCode == 13) {
                                layer.closeAll();
